...
vector<vector<int>> previous_matrix(n,temp);
set<vector<vector<int>>> invalid_check;// container consisting of all the states which are already traversed
class Game;
class game_logic
{
...
void game_logic::update()// updating every tile according to the move performed by the user and updating the score accordingly
{
	delta_score=0;
	break_game_signal=1;
...
int game_logic::game_stop()
{
	...
	for(int i=0;i<4;i++)
		for(int j=0;j<4;j++)
			if(previous_matrix[i][j]!=matrix[i][j])
			{	
				k=1;
				break;
			}
...
int main()
{
  ...
  sum_matrix=0;
    while(1)
    {
      ...
      if(sum_matrix>8)
      {	
      	if(invalid_check.count(matrix))// checking wether this state is already been traversed or not
      	{
      		game_main.undo();// if traversed not checking further
      	}
      	else
      	{
	  		game_main.undo();// if not, then checking it and printing it
	  	 	game_main.print_matrix();
	  	}
      }
      game_main.key_press();
